...
This Helm chart deploys the **OpenAirInterface Central Unit User Plane (OAI-CU-UP)**. 
...
| **nfimage.repository** | String | CUUP image name (default: `oaisoftwarealliance/oai-cuup`) |
...
| **multus.interfaces[].name** | `n3` / `f1u` / `e1` | Logical interface name inside the container |
...
| **config.cuupName** | CU-UP node name |
| **config.cucpHost** | CU-CP host name |
| **config.f1uPort** | f1u port number |
...
|------------|---------|-------------|
| **start.cuup** | `true` / `false` | If `false`, NF container sleeps (manual debug) |
