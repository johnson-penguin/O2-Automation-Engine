# Helm Chart for OAI Policy Control Function (PCF)

This Helm chart deploys the **OpenAirInterface Policy Control Function (OAI-PCF)**, compliant with **3GPP Release 16**.
...
OAI-PCF implements control-plane functions for 5G Core according to 3GPP Rel.16.  
More details about its features can be found on the [OAI-PCF Wiki](https://gitlab.eurecom.fr/oai/cn5g/oai-cn5g-pcf/-/wikis/home).

Source code: [GitLab – OAI PCF](https://gitlab.eurecom.fr/oai/cn5g/oai-cn5g-pcf)  
Container images: [Docker Hub – oaisoftwarealliance/oai-pcf](https://hub.docker.com/r/oaisoftwarealliance/oai-pcf)
...
│ ├── serviceaccount.yaml
│ └── service.yaml
├── config.yaml # PCF configuration (functional parameters)
...
3. **Deployment**
4. **ConfigMap** – holds the PCF configuration file
5. **ServiceAccount**
...
|------------|---------|-------------|
| **start.pcf** | `true` / `false` | If `false`, PCF container sleeps (manual debugging) |
...
| **resources.limits.nf.cpu / memory** | String | CPU/memory limits for PCF |
...
It is recommended to deploy PCF using one of the **parent OAI Helm charts**, which orchestrate multiple 5G Core NFs together:

1. [`oai-5g-basic`](../oai-5g-basic/README.md) – Minimal core deployment  
3. [`oai-5g-slicing`](../oai-5g-slicing/README.md) – Includes PCF for network slicing  

### Standalone Installation

```bash
helm install oai-pcf ./oai-pcf -n oai --create-namespace
```

To upgrade or update the configuration:

```
helm upgrade oai-pcf ./oai-pcf -n oai -f custom-values.yaml
```
To uninstall:

```
helm uninstall oai-pcf -n oai
```
...
1. **Tcpdump sidecar**: If `start.tcpdump: true` is enabled then also enable `persistent.sharedvolume: true` in both PCF and NRF charts
...
1. [OAI-PCF Source Code](https://gitlab.eurecom.fr/oai/cn5g/oai-cn5g-pcf)
2. [OAI Docker Hub Images](https://hub.docker.com/repository/docker/oaisoftwarealliance/oai-pcf)
