# Helm Chart for OAI Network function Repository Function (NRF)

This Helm chart deploys the **OpenAirInterface Network function Repository Function (OAI-NRF)**, compliant with **3GPP Release 16**.
...
OAI-NRF implements control-plane functions for 5G Core according to 3GPP Rel.16.  
More details about its features can be found on the [OAI-NRF Wiki](https://gitlab.eurecom.fr/oai/cn5g/oai-cn5g-nrf/-/wikis/home).

Source code: [GitLab – OAI NRF](https://gitlab.eurecom.fr/oai/cn5g/oai-cn5g-nrf)  
Container images: [Docker Hub – oaisoftwarealliance/oai-nrf](https://hub.docker.com/r/oaisoftwarealliance/oai-nrf)
...
│ ├── serviceaccount.yaml
│ └── service.yaml
├── config.yaml # NRF configuration (functional parameters)
...
3. **Deployment**
4. **ConfigMap** – holds the NRF configuration file
5. **ServiceAccount**
...
|------------|---------|-------------|
| **start.nrf** | `true` / `false` | If `false`, NRF container sleeps (manual debugging) |
...
| **resources.limits.nf.cpu / memory** | String | CPU/memory limits for NRF |
...
It is recommended to deploy NRF using one of the **parent OAI Helm charts**, which orchestrate multiple 5G Core NFs together:
...
```bash
helm install oai-nrf ./oai-nrf -n oai --create-namespace
```

To upgrade or update the configuration:

```
helm upgrade oai-nrf ./oai-nrf -n oai -f custom-values.yaml
```
To uninstall:

```
helm uninstall oai-nrf -n oai
```
...
1. **Tcpdump sidecar**: If `start.tcpdump: true` is enabled then also enable `persistent.sharedvolume: true` in both NRF and NRF charts
...
1. [OAI-NRF Source Code](https://gitlab.eurecom.fr/oai/cn5g/oai-cn5g-nrf)
2. [OAI Docker Hub Images](https://hub.docker.com/repository/docker/oaisoftwarealliance/oai-nrf)
