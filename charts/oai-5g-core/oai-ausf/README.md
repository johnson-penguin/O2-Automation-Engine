...
This Helm chart deploys the **OpenAirInterface Authentication Server Function (OAI-AUSF)**, compliant with **3GPP Release 16**.
...
OAI-AUSF implements control-plane functions for 5G Core according to 3GPP Rel.16.  
More details about its features can be found on the [OAI-AUSF Wiki](https://gitlab.eurecom.fr/oai/cn5g/oai-cn5g-ausf/-/wikis/home).

Source code: [GitLab – OAI AUSF](https://gitlab.eurecom.fr/oai/cn5g/oai-cn5g-ausf)  
Container images: [Docker Hub – oaisoftwarealliance/oai-ausf](https://hub.docker.com/r/oaisoftwarealliance/oai-ausf)
...
│ ├── serviceaccount.yaml
│ └── service.yaml
├── config.yaml # AUSF configuration (functional parameters)
...
3. **Deployment**
4. **ConfigMap** – holds the AUSF configuration file
5. **ServiceAccount**
...
|------------|---------|-------------|
| **start.ausf** | `true` / `false` | If `false`, AUSF container sleeps (manual debugging) |
...
| **resources.limits.nf.cpu / memory** | String | CPU/memory limits for AUSF |
...
It is recommended to deploy AUSF using one of the **parent OAI Helm charts**, which orchestrate multiple 5G Core NFs together:
...
```bash
helm install oai-ausf ./oai-ausf -n oai --create-namespace
```

To upgrade or update the configuration:

```
helm upgrade oai-ausf ./oai-ausf -n oai -f custom-values.yaml
```
To uninstall:

```
helm uninstall oai-ausf -n oai
```
...
1. **Tcpdump sidecar**: If `start.tcpdump: true` is enabled then also enable `persistent.sharedvolume: true` in both AUSF and NRF charts
...
1. [OAI-AUSF Source Code](https://gitlab.eurecom.fr/oai/cn5g/oai-cn5g-ausf)
2. [OAI Docker Hub Images](https://hub.docker.com/repository/docker/oaisoftwarealliance/oai-ausf)
