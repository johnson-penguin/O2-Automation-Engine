...
This Helm chart deploys the **OpenAirInterface Central Unit (OAI-CU)**.
...
**Note:**  
- For split deployments with dedicated interfaces (e.g., N2, N3 or F1), you’ll need the [Multus CNI plugin](https://github.com/k8snetworkplumbingwg/multus-cni).
...
The [OAI-CU](https://gitlab.eurecom.fr/oai/openairinterface5g/-/tree/develop) implements 5G NR base station functions per **3GPP Release 16**.  
...
| **multus.interfaces[].name** | `f1` / `n2` / `n3` | Logical interface name inside the container |
...
| **config.cuName** | CU name |
| **config.amfHost** | amf hostname or ip |
...
|------------|---------|-------------|
| **start.cu** | `true` / `false` | If `false`, NF container sleeps (manual debug) |
