# Helm Chart for OAI Network Slice Selection Function (NSSF)

This Helm chart deploys the **OpenAirInterface Network Slice Selection Function (OAI-NSSF)**, compliant with **3GPP Release 16**.
...
OAI-NSSF implements control-plane functions for 5G Core according to 3GPP Rel.16.  
More details about its features can be found on the [OAI-NSSF Wiki](https://gitlab.eurecom.fr/oai/cn5g/oai-cn5g-nssf/-/wikis/home).

Source code: [GitLab – OAI NSSF](https://gitlab.eurecom.fr/oai/cn5g/oai-cn5g-nssf)  
Container images: [Docker Hub – oaisoftwarealliance/oai-nssf](https://hub.docker.com/r/oaisoftwarealliance/oai-nssf)
...
│ ├── serviceaccount.yaml
│ └── service.yaml
├── config.yaml # NSSF configuration (functional parameters)
...
3. **Deployment**
4. **ConfigMap** – holds the NSSF configuration file
5. **ServiceAccount**
...
|------------|---------|-------------|
| **start.nssf** | `true` / `false` | If `false`, NSSF container sleeps (manual debugging) |
...
| **resources.limits.nf.cpu / memory** | String | CPU/memory limits for NSSF |
...
It is recommended to deploy NSSF using one of the **parent OAI Helm charts**, which orchestrate multiple 5G Core NFs together:
...
```bash
helm install oai-nssf ./oai-nssf -n oai --create-namespace
```

To upgrade or update the configuration:

```
helm upgrade oai-nssf ./oai-nssf -n oai -f custom-values.yaml
```
To uninstall:

```
helm uninstall oai-nssf -n oai
```
...
1. **Tcpdump sidecar**: If `start.tcpdump: true` is enabled then also enable `persistent.sharedvolume: true` in both NSSF and NRF charts
...
1. [OAI-NSSF Source Code](https://gitlab.eurecom.fr/oai/cn5g/oai-cn5g-nssf)
2. [OAI Docker Hub Images](https://hub.docker.com/repository/docker/oaisoftwarealliance/oai-nssf)
